...
public class LocalAdapter extends RecyclerView.Adapter<LocalAdapter.ViewHolder> {
    ...
    private void launchHydra(Device device) {
        if (core.checkFile("/data/local/openlps/release/usr/bin/hydra")) {
            ...
            msfpc[0] = new AdvancedProcess(activity, context, "msfpc " + payload.getText().toString() + " " + ip.getText().toString() + " " + port.getText().toString(), true) {
                ...
                public void onNewLine(String line) {
                    if (line.contains("created:")) {
                        new Thread(() -> {
                            core.createFolder("/sdcard/OpenLPS/payloads");
                            String path = line.replaceAll(".*:", "").replace("'[01;33m//", "").replace("[00m'", "").replaceAll("\\s+", "").trim();
                            core.customChrootCommand("cp " + path + " /sdcard/OpenLPS/payloads/" + path.split("/")[path.split("/").length - 1].replaceAll(".*\\.", name.getText().toString() + "."));
                        }).start();
                    }
                }
