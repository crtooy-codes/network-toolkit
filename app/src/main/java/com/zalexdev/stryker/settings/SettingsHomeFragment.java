...
public class SettingsHomeFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        LinearLayout language = view.findViewById(R.id.language_view);
    ...
    private void updateLanguageValue(TextView display) {
        String selected = AppCompatDelegate.getApplicationLocales().toLanguageTags();
        String[] tags = getResources().getStringArray(R.array.openlps_language_tags);
        String[] labels = getResources().getStringArray(R.array.openlps_language_labels);
        int index = 0;
        for (int i = 1; i < tags.length; i++) {
            if (selected.equalsIgnoreCase(tags[i])
                    || selected.toLowerCase().startsWith(tags[i].toLowerCase() + "-")) {
                index = i;
                break;
            }
        }
        display.setText(labels[index]);
    }
    ...
    private void performDelete() {
        ...
        new Thread(() -> {
            ...
            core.customCommand("rm -rf /data/local/openlps");
            ui(() -> {
            ...
            core.customCommand("pm uninstall " + BuildConfig.APPLICATION_ID);
            ui(() -> {
