            ...
            handshake = new AdvancedThread(activity,context) {
                ...
                public void doOnBackground() {
                    ...
                    if (monitor && monitor2){
                        ...
                        new Thread(() -> {
                            String cmd = "airodump-ng " + core.getHSInterface() + " -w /sdcard/OpenLPS/hs/handshake  --ignore-negative-one --output-format pcap -c "+network.getChannel()+" --bssid " + network.getMac()+" --update 3";
                            if (network.getIs5hhz()){
                                cmd = "airodump-ng " + core.getHSInterface() + " -w /sdcard/OpenLPS/hs/handshake --ignore-negative-one --output-format pcap  --bssid " + network.getMac() + " --band a --update 3";
                            }
                        ...
                        while (!airoRunning[0]){
                            ...
                            if (hsStatus[0]) {
                                ...
                                sendEvent("Handshake saved to /sdcard/OpenLPS/captured/" + filename);
                                ...
                                core.moveFile(core.getStorage() + "Stryker/hs/handshake-01.cap", core.getStorage() + "Stryker/captured/" + filename);
                                sendEvent("PMKID saved to /sdcard/OpenLPS/captured/" + filename);
                                com.zalexdev.stryker.geomac.GeoHooks.recordHandshake(
                                        context, network.getMac(), network.ssid);
                                activity.runOnUiThread(this::onFinished);
                            }
