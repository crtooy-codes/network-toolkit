...
public class Slide3 extends Fragment {
    ...
    private static final String DOWNLOADED_CHROOT_PATH =
            "/data/data/com.openlps.networktoolkit/files/core.tar.gz";
    ...
    private void startInstall() {
        ...
        resetStages();
        setStatus(StatusKind.RUNNING, getString(R.string.setup_core_card_title),
                getString(R.string.setup_core_status_starting));
        log(LogLevel.INFO, "Architecture: " + (core.is64Bit() ? "64-bit (arm64-v8a)" : "32-bit (armeabi-v7a)"));
        log(LogLevel.INFO, "OpenLPS " + BuildConfig.VERSION_NAME + " · build " + BuildConfig.VERSION_CODE);
        ...
        new Thread(() -> {
            ...
            if (downloadChroot(chrootAsset)) {
                ...
                log(LogLevel.STEP, "Extracting archive into /data/local/openlps");
                ...
                if (unTarFile()) {
                    ...
                    core.deleteFile("/sdcard/OpenLPS/exploits/");
                    core.copyFile("/data/data/com.openlps.networktoolkit/files/checker.py",
                            "/data/local/openlps/release/exploits/checker.py");
                    core.copyFile("/data/local/openlps/release/exploits/", "/sdcard/OpenLPS/exploits");
                    ...
                    log(LogLevel.INFO, "Copied " + authCopied + " auth_*.txt list"
                            + (authCopied == 1 ? "" : "s") + " to /sdcard/OpenLPS/rs");
                    markStage(InstallStage.DEPLOYING_EXPLOITS, RowState.DONE);
                    log(LogLevel.SUCCESS, "Exploits deployed to /sdcard/OpenLPS/exploits");

                    markStage(InstallStage.FINALIZING, RowState.ACTIVE);
                    log(LogLevel.CMD, "echo update > /data/local/openlps/release/4.0");
                    core.customCommand("echo update > /data/local/openlps/release/4.0");
                    core.deleteFile("/sdcard/OpenLPS/exploits/checker.py");
                    ...
                    setStatus(StatusKind.SUCCESS, getString(R.string.setup_core_card_title),
                            getString(R.string.setup_core_status_complete));
    ...
    private void failWith(String reason) {
        setStatus(StatusKind.FAILED, getString(R.string.setup_core_card_title), reason);
    ...
    private void clear() {
        core.chmodFolder("/data/data/com.openlps.networktoolkit/files/");
        core.createFolder(OpenLpsPaths.SHARED_ROOT);
        core.createFolder("/data/local/openlps");
        if (core.checkFolder("/data/local/openlps/release/sdcard/OpenLPS")) {
            core.unmountCore();
        }
        core.deleteFile("/data/local/openlps/release");
        core.deleteFile(OpenLpsPaths.SHARED_ROOT + "/release");
        core.deleteFile(core.getStorage() + "Download/OpenLPS.apk");
        core.createFolder(OpenLpsPaths.SHARED_ROOT + "/hs");
        core.createFolder(OpenLpsPaths.SHARED_ROOT + "/captured");
        core.createFolder(OpenLpsPaths.SHARED_ROOT + "/exploits");
        core.createFolder(OpenLpsPaths.SHARED_ROOT + "/wordlists");
        core.createFolder(OpenLpsPaths.SHARED_ROOT + "/reports");
        core.createFolder(OpenLpsPaths.SHARED_ROOT + "/rs");
    }
    ...
    private boolean unTarFile() {
        ...
        notificationManager.notify(NOTIFICATION_ID, notification.build());
        core.customCommand(Core.BUSYBOX + " tar -xzf " + DOWNLOADED_CHROOT_PATH + " -C /data/local/openlps/");
        return core.checkFolder("/data/local/openlps/release/bin/");
    }
