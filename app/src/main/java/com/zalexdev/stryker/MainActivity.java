...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (core.getBoolean("clean_after_update")) {
            new Thread(() -> core.deleteFile("/data/data/com.openlps.networktoolkit/files/OpenLPS.apk")).start();
            core.remove("clean_after_update");
        }

        if (core != null) {
            if (!core.getBoolean("first_open") || !core.checkFile("/data/local/openlps/release/4.0")) {
                core.putString("username", "User");
                Intent install = new Intent(this, AppIntroActivity.class);
                startActivity(install);
            } else {
                new Thread(() -> {
                    if (!core.checkFolder("/data/local/openlps/release/usr")) {
                        Intent install = new Intent(this, AppIntroActivity.class);
                        ...
                        if (core.isMounted()) {
                            ...
                            new Thread(() -> {
                                ...
                                assert core != null;
                                core.chmodFolder("/data/data/com.openlps.networktoolkit/files");
                            }).start();
                        } else {
                            if (!core.mountCore()) {
        ...
        new Thread(() -> {
            ...
            runOnUiThread(() -> {
                if (chrootStatus != null) {
                    chrootStatus.setText(mounted
                            ? R.string.drawer_chroot_mounted
                            : R.string.drawer_chroot_detached);
                }
            });
        }).start();
    ...
    private void copyAssets() {
        ...
        if (files != null) for (String filename : files) {
            ...
            try {
                ...
                File outFile = new File("/data/data/com.openlps.networktoolkit/files/", filename);
    ...
    private void copyAssets2() {
        ...
        if (files != null) for (String filename : files) {
            ...
            try {
                in = assetManager.open(filename);
                File outFile = new File("/data/data/com.openlps.networktoolkit/files/imgs_adapters", filename);
                out = new FileOutputStream(outFile);
                copyFile(in, out);
            } catch (IOException ignored) {
            } finally {
    ...
    public static class Receiver implements MainActivityReceiver {
        ...
        public void restoreTitle() {
            logo.setText(R.string.brand_short);
        }
    ...
    private static java.util.LinkedHashMap<Integer, DrawerSpec> buildDrawerSpecs() {
        ...
        m.put(R.id.dasboard_item,     new DrawerSpec(R.string.drawer_dashboard,       R.drawable.home,        0xFF1565C0));
        m.put(R.id.terminal_item,     new DrawerSpec(R.string.drawer_terminal,        R.drawable.terminal,    0xFF1565C0));
        m.put(R.id.wifi_item,         new DrawerSpec(R.string.drawer_wifi,            R.drawable.wifi,        0xFF1565C0));
        m.put(R.id.hs_item,           new DrawerSpec(R.string.drawer_handshakes,      R.drawable.storage,     0xFF00897B));
        m.put(R.id.macchanger_item,   new DrawerSpec(R.string.drawer_macchanger,      R.drawable.password,    0xFF1565C0));
        m.put(R.id.router_scan_item,  new DrawerSpec(R.string.drawer_router_scan,     R.drawable.router,      0xFF2E7D32));
        m.put(R.id.wpair_item,        new DrawerSpec(R.string.drawer_wpair,           R.drawable.wpair,       0xFF3949AB));
        m.put(R.id.lan_item,          new DrawerSpec(R.string.drawer_local_network,   R.drawable.lan,         0xFFAB47BC));
        m.put(R.id.nmap_item,         new DrawerSpec(R.string.drawer_nmap,            R.drawable.scanner,     0xFFAB47BC));
        m.put(R.id.nuclei_item,       new DrawerSpec(R.string.drawer_web_scanner,     R.drawable.webscan,     0xFF3949AB));
        m.put(R.id.arsenal_item,      new DrawerSpec(R.string.drawer_arsenal,         R.drawable.motion_blur, 0xFFEF6C00));
        m.put(R.id.hid_item,          new DrawerSpec(R.string.drawer_hid,             R.drawable.keyboard,    0xFFC62828));
        m.put(R.id.metasploit_item,   new DrawerSpec(R.string.drawer_metasploit,      R.drawable.shield,      0xFFC62828));
        m.put(R.id.geomac_item,       new DrawerSpec(R.string.drawer_geomac,          R.drawable.map,         0xFF00838F));
        m.put(R.id.vnc_item,          new DrawerSpec(R.string.drawer_vnc,             R.drawable.vnc,         0xFF5E35B1));
        m.put(R.id.usb_arsenal_item,  new DrawerSpec(R.string.drawer_usb_arsenal,     R.drawable.usb,         0xFF1565C0));
        m.put(R.id.manager_item,      new DrawerSpec(R.string.drawer_core_manager,    R.drawable.tune,        0xFF5E35B1));
        m.put(R.id.about_item,        new DrawerSpec(R.string.drawer_about,           R.drawable.info_outlined, 0xFF1565C0));
        return m;
    }
}
